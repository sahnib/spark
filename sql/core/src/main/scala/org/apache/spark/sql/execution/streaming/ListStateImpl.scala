...
class ListStateImpl[S](store: StateStore,
  ...
  extends ListState[S] with Logging {
   ...
   override def exists(): Boolean = {
     val encodedGroupingKey = StateTypesEncoderUtils.encodeGroupingKey(stateName, keyExprEnc)
   ...
   override def get(): Iterator[S] = {
     val encodedKey = StateTypesEncoderUtils.encodeGroupingKey(stateName, keyExprEnc)
   ...
   override def put(newState: Array[S]): Unit = {
     ...
     if (newState.isEmpty) {
       ...
       var isFirst = true
       newState.foreach { v =>
         val encodedValue = StateTypesEncoderUtils.encodeValue(v)
         if (isFirst) {
   ...
   override def appendValue(newState: S): Unit = {
     ...
     store.merge(StateTypesEncoderUtils.encodeGroupingKey(stateName, keyExprEnc),
         StateTypesEncoderUtils.encodeValue(newState), stateName)
   }
   ...
   override def clear(): Unit = {
     store.remove(StateTypesEncoderUtils.encodeGroupingKey(stateName, keyExprEnc), stateName)
   }
